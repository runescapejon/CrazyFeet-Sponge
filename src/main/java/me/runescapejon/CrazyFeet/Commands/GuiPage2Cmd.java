...
public class GuiPage2Cmd implements CommandExecutor {
	...
	public void onInventoryClick(ClickInventoryEvent event, @First Player player) {
		if (event.getTargetInventory().getName().get().equals(this.invs.getName().get())) {
			...
			if (item.getItem().equals(ItemTypes.ENCHANTED_BOOK)) {
			...
			if (item.getItem().equals(ItemTypes.BOOK)) {
			...
			if (item.getItem().equals(ItemTypes.SLIME_BALL)) {
			...
			if (item.getItem().equals(ItemTypes.WATER_BUCKET)) {
	...
	public CommandResult execute(CommandSource src, CommandContext ctx) throws CommandException {
		...
		invs.query(SlotPos.of(1, 1)).set(mhelix);
		invs.query(SlotPos.of(2, 1)).set(globe);
		invs.query(SlotPos.of(3, 1)).set(storm);
