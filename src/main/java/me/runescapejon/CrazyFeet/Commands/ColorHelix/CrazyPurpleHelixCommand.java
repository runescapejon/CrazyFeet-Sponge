		...
		if (!target.isPresent()) {
			...
			if (player.hasPermission("crazyFeet.crazypurplehelix")) {
				if (cPurplehelix.contains(player.getUniqueId())) {
					...
					player.sendMessage(
							LanguageUtils.getText("crazyPurpleHelixDisabled", new Pair<>("%PLAYER%", player.getName())));
			...
			if (cPurplehelix.contains(targ.getUniqueId())) {
				...
				targ.sendMessage(
						LanguageUtils.getText("crazyPurpleHelixEnabledByPlayer", new Pair<>("%PLAYER%", src.getName())));
